...
export function ExtendVideo() {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const newJob = await api.extendVideo({ videoId: id, prompt: prompt.trim(), duration });
      toast(t('extend.title') + ' - OK', 'success');
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <h2 className="text-2xl font-bold">{t('extend.title')}</h2>

      {job && job.status === 'completed' && (
        <div className="space-y-2">
          <label className="text-sm font-medium text-zinc-300">{t('extend.sourceVideo')}</label>
          <video controls className="w-full rounded-lg border border-zinc-800" src={api.getVideoContentUrl(job.id)} />
        </div>
      )}

      <div className="space-y-2">
        <label className="text-sm font-medium text-zinc-300">{t('extend.prompt')}</label>
        <textarea
          value={prompt}
          onChange={e => setPrompt(e.target.value)}
          placeholder={t('extend.promptPlaceholder')}
          rows={3}
      ...
      <div className="space-y-2">
        <label className="text-sm font-medium text-zinc-300">{t('extend.duration')}</label>
        <select
          value={duration}
